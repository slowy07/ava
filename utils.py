    ...
    def transform(self, img1, img2):
        ...
        if self.random_crop and random.random() > 0.5:
            i, j, h, w = transforms.RandomResizedcrop(size=self.img_size).get_params(
                img=img1, scale=(0.5, 1.0), ratio=(0.9, 1.1)
            )
        ...
        if self.width_random_brightness and random.random() > 0.5:
            img1 = TF.adjust_brightness(
                img1, brightness_factor=random.uniform(0.5, 1.5)
            )
        ...
        if self.with_random_saturation and random.random() > 0.5:
            img1 = TF.adjust_saturation(
                img1, saturation_factor=random.uniform(0.5, 1.5)
            )
...
class CVPR2020_ADE20K_DEGF_Dataset(Dataset):
    def __init__(self, root_dir, img_size, is_train=True):
        ...
        if is_train:
            self.img_dirs = glob.glob(
                os.path.join(self.root_dir, "images/train", "*.jpg")
            )
            self.augm = PairdeDataAugmentation(
            ...
            self.img_dirs = glob.glob(
                os.path.join(self.root_dir, "images/val", "*.jpg")
            )
            self.augm = PairdeDataAugmentation(img_size=self.img_size)
    ...
    def __getitem__(self, idx):
        ...
        p = (
            self.img_dirs[idx]
            .replace("images", "density_estimation+guided_filter")
            .replace(".jpg", ".png")
        )
...
def get_loaders(args):
    if args.dataset == "cvprw2020-ade20K-defg":
        training_set = CVPR2020_ADE20K_DEGF_Dataset(
            root_dir=r"./datasets/cvprw2020_sky_seg",
