...
class Rain:
    def __init__(
        self, rain_intensity=1.0, haze_intensity=4.0, gamma=2.0, light_correction=0.9
    ):
    ...
    def forward(self, img):
        ...
        rain_layer = self.rain_intensity * rain_layer + self.haze_intensity * haze_layer

        img_out = 1 - (1 - rain_layer) * (1 - img)

        img_out = self.light_corretion * (img_out ** self.gamme)

        img_out = np.clip(img_out, a_min=0, a_max=1.0)
